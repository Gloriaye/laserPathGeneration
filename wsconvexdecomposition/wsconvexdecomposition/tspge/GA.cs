...
namespace wsconvexdecomposition
{
    ...
    public static Population evolvePopulation(Population pop) {
        Population newPopulation = new Population(pop.populationSize(), false, pop.oripaths, pop.startPoint);   //构造新种群
        // 第一个位置 保持最优个体
        int elitismOffset = 0;
        if (elitism) {
    ...
    // Selects candidate tour for crossover   //随机选取tournamentSize条tour，返回最好的。适应度高返回的概率大
    private static Tour tournamentSelection(Population pop) {
        // Create a tournament population
        Population tournament = new Population(tournamentSize, false, pop.oripaths, pop.startPoint);
        // For each place in the tournament get a random candidate tour and
        // add it
        for (int i = 0; i < tournamentSize; i++) {
            int randomId = (int)(new Random().NextDouble() * pop.populationSize());
